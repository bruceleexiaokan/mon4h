...
public class InternalMetricsTimer extends Thread{
	private static final Logger log = LoggerFactory.getLogger(InternalMetricsTimer.class);
		...
		while(!exit){
			...
			try{
				...
				if(now-maxLatencyUptime>maxLatencyInterval){
					...
					Stats.latencyGetMetricsTagsCmd.clean(now, maxLatencyInterval);
					Stats.latencyGetDataPointsCmd.clean(now, maxLatencyInterval);
					Stats.latencyGetGroupedDataPointsCmd.clean(now, maxLatencyInterval);
					Stats.latencyPutDataPointsCmd.clean(now, maxLatencyInterval);
			...
			try{
				...
				if(Config.systemType == Config.SYSTEM_TYPE_QUERY){
				...
				if(Config.systemType == Config.SYSTEM_TYPE_PUSH){
					tags = new HashMap<String,String>();
					tags.put("appid", "920701");
					tags.put("statstype", "total_cmd_count");
					tags.put("cmdtype", "PutDataPoints");
					value = Stats.putDataPointsCmdCount.get();
					writer.log(metrics, value, tags, new Date(curTime));
					...
					tags.put("statstype", "latest_min_cmd_count");
					tags.put("cmdtype", "PutDataPoints");
					value = Stats.timedPutDataPointsCmdInfo.getCmdSum();
					writer.log(metrics, value, tags, new Date(curTime));
					...
					tags.put("statstype", "latest_min_cmd_avg_latency");
					tags.put("cmdtype", "PutDataPoints");
					value = Stats.timedPutDataPointsCmdInfo.getAvgLatency();
					writer.log(metrics, value, tags, new Date(curTime));
				}
			}catch(Exception e){
