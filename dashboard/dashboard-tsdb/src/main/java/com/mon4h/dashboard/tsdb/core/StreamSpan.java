...
public final class StreamSpan implements DataPoints {
  ...
  public static void setLocalCache(LocalCache localCache){
	  StreamSpan.localCache = localCache;
  }
  ...
  public void addRowAndStreamDownsample(final KeyValue row){
	  ...
	  List<CachedTimeSeries> list = new LinkedList<CachedTimeSeries>();
	  CachedTimeSeries ts = new CachedTimeSeries();
	  ts.tsk = ISO8859BytesToString(removeSpanKeyTimeStamp(spanKey));
	  ts.timestamps = new LinkedList<CachedDataPoint>();
		  ...
		  while(it.hasNext()){
			  ...
			  if(last_dp_ts<sampleTimePoints.get(curTimePoint)){
				  ...
				  if(idp.isInteger()){
					  ...
					  ts.timestamps.add(setDataPoint(idp,true));
				  }else{
					  intervalIsInteger = false;
					  intervalDataPoints.addDouble(idp.doubleValue());
					  ts.timestamps.add(setDataPoint(idp,false));
				  }
  ...
  private CachedDataPoint setDataPoint( DataPoint idp,boolean LongOrDouble ) {

	  CachedDataPoint dp = new CachedDataPoint();
	  dp.timestamp = idp.timestamp();
	  dp.data = new CachedVariableData();
	  if( LongOrDouble ) {
		  dp.data.setType( (byte)CachedVariableData.VariableLong );
		  dp.data.setLong( idp.longValue() );
	  } else {
		  dp.data.setType( (byte)CachedVariableData.VariableDouble );
		  dp.data.setDouble( idp.doubleValue() );
	  }
	  return dp;
  }
