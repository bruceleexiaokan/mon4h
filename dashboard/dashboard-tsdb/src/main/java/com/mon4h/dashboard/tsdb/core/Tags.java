...
import com.mon4h.dashboard.tsdb.uid.NoSuchUniqueId;
import com.mon4h.dashboard.tsdb.uid.NoSuchUniqueName;
...
public final class Tags {
    ...
    static String getValue(final TSDB tsdb, final byte[] row,
        ...
        final byte[] id = UniqueIds.tag_names().getId(name);
        ...
        try {
            return UniqueIds.tag_values().getName(value_id);
     ...
     */
    public static byte[] getValueId(final TSDB tsdb, final byte[] row,
                                    final byte[] tag_id) {
        ...
        for (short pos = (short) (UniqueIds.metrics().width() + Const.TIMESTAMP_BYTES);
             pos < row.length;
     ...
     *         tag ID isn't present in this row key.
     */
    public static byte[] getValueId(final byte[] row,
                                    final byte[] tag_id, int metric_width, int name_width, int value_width) {
        // TODO(tsuna): Can do a binary search.
        for (short pos = (short) (metric_width + Const.TIMESTAMP_BYTES);
             pos < row.length;
             pos += name_width + value_width) {
            if (rowContains(row, pos, tag_id)) {
                pos += name_width;
    ...
    static Map<String, String> getTags(final TSDB tsdb,
        ...
        final short metric_ts_bytes = (short) (UniqueIds.metrics().width()
        ...
        for (short pos = metric_ts_bytes; pos < row.length; pos += tag_bytes) {
            ...
            final String name = UniqueIds.tag_names().getName(tmp_name);
            System.arraycopy(row, pos + name_width, tmp_value, 0, value_width);
            final String value = UniqueIds.tag_values().getName(tmp_value);
...
//    }
    ...
    static ArrayList<byte[]> resolveAllInternal(final TSDB tsdb,
        ...
        for (final Map.Entry<String, String> entry : tags.entrySet()) {
            final byte[] tag_id = (create
                    ? UniqueIds.tag_names().getOrCreateId(entry.getKey())
                    : UniqueIds.tag_names().getId(entry.getKey()));
            final byte[] value_id = (create
                    ? UniqueIds.tag_values().getOrCreateId(entry.getValue())
                    : UniqueIds.tag_values().getId(entry.getValue()));
     ...
     *                                  the wrong number of bytes.
     */
    static List<String> resolveNames(final TSDB tsdb,
                                     final ArrayList<byte[]> tags)
            throws NoSuchUniqueId {
        final short name_width = UniqueIds.tag_names().width();
        final short tag_bytes = (short) (name_width);
        final byte[] tmp_name = new byte[name_width];
