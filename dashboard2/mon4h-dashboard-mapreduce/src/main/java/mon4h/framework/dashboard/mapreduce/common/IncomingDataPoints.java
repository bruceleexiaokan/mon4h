...
//  private boolean batch_import;
  ...
  static byte[] rowKeyTemplate(final TSDB tsdb,
    ...
    final short tag_name_width = UniqueIds.tag_names().width();
    final short tag_value_width = UniqueIds.tag_values().width();
    ...
    copyInRowKey(row, pos, (AUTO_METRIC ? UniqueIds.metrics().getOrCreateId(metric)
                       : UniqueIds.metrics().getId(metric)));
    pos += metric_width;
  ...
  private long updateBaseTime(final long timestamp) {
    ...
    Bytes.setInt(row, (int) base_time, UniqueIds.metrics().width());
...
//    return tsdb.client.put(point)/*.addBoth(cb)*/;
  ...
  private long baseTime() {
    return Bytes.getUnsignedInt(row, UniqueIds.metrics().width());
  }

  public void addPoint(final long timestamp, final long value) {
    final short flags = 0x7;  // An int stored on 8 bytes.
    addPointInternal(timestamp, Bytes.fromLong(value), flags);
  }

  public void addPoint(final long timestamp, final float value) {
