...
public class ConfigureNode {
			...
			if(path.size()>0){
				for(String pathItem:path){
					...
					if(targetNode.childs != null){
						for(ConfigureNode child:targetNode.childs){
	...
	public ConfigureNode getConfigNode(String cfgName) throws NoSuchNodeException{
		ConfigureNode targetNode = this;
		if(cfgName != null){
			List<String> path = parsePath(cfgName);
			if(path.size()>0){
