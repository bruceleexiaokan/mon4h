...
public class RegisterAdd {
	...
	public void processrequest(HttpServletRequest request,
		...
		try {
			...
			String[] inputs = request.getParameterMap().get("registerinput");
			String[] texts = request.getParameterMap().get("registerinputjs");
			...
			if( input != null && input.length() != 0 ) {
				...
				for( String str : content ) {
					RegisterConfig.add(str,RegisterConfig.getKey());
				}
			} else if( text != null && text.length() != 0 ) {
				String[] jscontent = text.split("=====");
				if( jscontent != null ) {
					for( String str : jscontent ) {
						String[] temp = str.split("====");
						if( temp != null ) {
							RegisterConfig.addJs(temp[0],temp[1],RegisterConfig.getKey());
						}
					}
				}
			}
